...
- **Outbound (UDP):** The FMS sends control commands to robots every 500ms via UDP port 8080, using team IP convention: team 1234 AŸ+' 10.12.34.5
- **HTTP Status:** Health check endpoint at localhost:2000/status

**Core Design:**
```
TCP Listener AŸ+' Parse packets AŸ+' Update shared FMS state (Arc<Mutex<FMS>>)
                                        AŸ+"
                                   Per-team UDP workers AŸ+' Send control commands to robots
```
...
- **Minimize lock contention:** Acquire the Mutex, make changes quickly, and releaseAŸ?"don't hold locks across await points
...
### Module Organization
- **`tcp.rs`** AŸ?" TCP packet parsing and protocol definitions
- **`udp.rs`** AŸ?" UDP packet creation and robot control commands
- **`driverstation_connection.rs`** AŸ?" DriverStation connection state and handlers
- **`fms.rs`** AŸ?" Central FMS state and logic
- **`main.rs`** AŸ?" Entry point, socket initialization, async runtime setup
